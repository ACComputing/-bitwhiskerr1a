...
- Dual-Language Heuristics (English & Mandarin)
- ✨ DeepSeek R1‑style “Aha Moment” reasoning chain ✨
"""
...
def offline_distilled_stream(prompt, stop_evt, include_reasoning=True):
    """
    A generator that mimics a fast, direct offline model response.
    Includes explicit heuristics to detect Mandarin vs English.
    If include_reasoning is True, it first emits a reasoning chain.
    """
    p = prompt.lower()
    ...
    # Heuristic to detect Mandarin (checks exclusively for CJK Unified Ideographs - Chinese)
    is_mandarin = any('\u4e00' <= char <= '\u9fff' for char in prompt)
    
    # ── Determine final answer first (original logic) ───────────────────────
    final_response = ""
    
    if is_mandarin:
        if any(w in p for w in ["你好", "嗨", "您好", "哈喽"]):
            final_response = "你好！我是 BitWhisker R1 1.x，在完全离线的环境下运行。我能为您写代码或聊天提供什么帮助？"
            
        elif any(w in p for w in ["代码", "python", "程序", "脚本", "函数", "写", "素数"]):
            final_response = (
                "没问题！这里有一段纯 Python 脚本，您可以在沙盒中测试。\n"
                "它计算前 N 个素数：\n\n"
                "```python\n"
                "def get_primes(n):\n"
                "    \"\"\"生成前 n 个素数。\"\"\"\n"
                ...
                "print(f'结果: {result}')\n"
                "```\n\n"
                "我已经将这段代码直接加载到了右侧的沙盒中。点击 **▶ Run Code** 即可执行！"
            )
            
        elif any(w in p for w in ["笑话", "好笑", "开心"]):
            jokes = [
                "为什么 Python 程序员喜欢深色模式？因为浅色会吸引 Bug (虫子)！",
                "世界上有 10 种人：懂二进制的，和不懂的。",
                "程序员睡觉前会在床头柜上放两个水杯。一个是满的，以防口渴；一个是空的，以防不渴。"
            ]
            final_response = random.choice(jokes)
            
        elif any(w in p for w in ["数学", "计算", "方程"]):
            final_response = (
                "我具备编写算法解决数学问题的能力。您可以让我编写一个 Python 函数来解决"
                "特定的方程式，然后您可以在内置的沙盒中直接运行它！"
            )
                              
        elif any(w in p for w in ["你是谁", "什么", "版本", "名字"]):
            final_response = (
                "我是 BitWhisker R1 1.x，一个内置了 Python 沙盒的离线精简模型。"
                "我完全在您的本地机器上生成直接、有用的回复和代码。"
            )
                              
        else:
            final_response = (
                "我正在通过本地的 BitWhisker R1 1.x 路径处理您的输入。"
                "您想一起探索什么特定主题或代码？"
            )
            
    else: # English Logic
        if any(w in p for w in ["hi", "hello", "hey", "greetings", "sup"]):
            final_response = "Hello! I am BitWhisker R1 1.x, operating completely offline. How can I help you with your coding or chat tasks today?"
    ...
    if include_reasoning:
        if is_mandarin:
            # Mandarin reasoning templates
            if any(w in p for w in ["代码", "python", "程序", "脚本", "函数", "写", "素数"]):
                reasoning_text = (
                    "💭 让我先理清思路：用户想要一段 Python 代码。\n"
                    "💭 我需要选择一个常见且实用的示例。素数生成是一个经典问题。\n"
                    "💭 算法可以很简单：从2开始，检查每个数是否是素数，直到收集到所需数量。\n"
                    "💭 为了提高效率，可以只检查到平方根。\n"
                    "💭 嗯，这样实现既清晰又能演示基础循环和条件判断。\n"
                    "💭 好了，现在我可以给出代码了。\n"
                )
            elif any(w in p for w in ["数学", "计算", "方程"]):
                reasoning_text = (
                    "💭 用户询问数学计算。我需要提供一个能解决数学问题的方案。\n"
                    "💭 可以建议编写一个 Python 函数，例如求解二次方程。\n"
                    "💭 或者展示数值积分方法。我会在回答中说明。\n"
                    "💭 这样用户就能在沙盒里尝试了。\n"
                )
            elif any(w in p for w in ["笑话", "好笑", "开心"]):
                reasoning_text = (
                    "💭 用户想听笑话。我需要选一个既有趣又不冒犯的笑话。\n"
                    "💭 技术相关或程序员梗可能比较合适。\n"
                    "💭 让我回忆一下经典的笑话库。\n"
                    "💭 嗯，这个不错，应该能逗乐用户。\n"
                )
                ...
                reasoning_text = (
                    "💭 让我理解用户的意图。\n"
                    "💭 根据输入内容，我需要给出一个有帮助的回应。\n"
                    "💭 考虑到离线环境，我的回答要简洁明了。\n"
                    "💭 好了，现在我可以回答了。\n"
                )
        else:
            # English reasoning templates
            if any(w in p for w in ["code", "python", "program", "script", "function", "write", "sort", "prime"]):
                reasoning_text = (
                    "💭 Let me reason step by step: the user wants a Python code example.\n"
                    "💭 I'll choose a classic problem: generating prime numbers. It's simple yet illustrative.\n"
                    "💭 The algorithm: start from 2, test each number for primality by checking divisibility up to its square root.\n"
                    "💭 I'll structure it as a function get_primes(n) that returns a list of the first n primes.\n"
                    "💭 Then I'll include a demo print. That should be clear and useful.\n"
                    "💭 Alright, I'm ready to present the code.\n"
                )
            elif "math" in p or "calculate" in p or "equation" in p:
                reasoning_text = (
                    "💭 The user is asking about math/calculation. I should suggest a Python solution.\n"
                    "💭 I can mention writing a function to solve quadratic equations or integrate numerically.\n"
                    "💭 That way they can test it in the sandbox.\n"
                )
            elif "joke" in p or "funny" in p:
                reasoning_text = (
                    "💭 The user wants a joke. I'll pick a tech-related or programmer joke.\n"
                    "💭 Let me recall a classic one that's lighthearted.\n"
                    "💭 Yes, that one will work.\n"
                )
            else:
                reasoning_text = (
                    "💭 Let me parse the user's input.\n"
                    "💭 I need to generate a helpful, offline-friendly response.\n"
                    "💭 Considering the context, I'll produce a concise and informative answer.\n"
                    "💭 Ready.\n"
                )
    
    # ── Streaming ───────────────────────────────────────────────────────────
    # 1. First stream reasoning (if enabled and not empty)
    if reasoning_text:
        # Stream character by character for better simulation
        for char in reasoning_text:
            if stop_evt.is_set():
                return
            yield char
            time.sleep(random.uniform(0.005, 0.03))
        # Add a small pause before final answer
        time.sleep(0.1)
    
    # 2. Then stream final response
    if is_mandarin:
...
class BitWhiskerApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("BitWhisker R1 1.x Sandbox (with Aha Moment)")
        ...
        self._show_reasoning = tk.BooleanVar(value=True)  # toggle reasoning display
    ...
    def _build_ui(self):
        ...
        tk.Checkbutton(chat_tools, text="💭 Show Reasoning", variable=self._show_reasoning,
                       bg=BG_MAIN, fg=FG_DIM, selectcolor=BG_MAIN,
                       font=FONT_SMALL, relief="flat", cursor="hand2").pack(side="left", padx=8)
        ...
        self.chat.tag_config("reasoning",  foreground=FG_DIM,  font=("Segoe UI", 9, "italic"))
    ...
    def _post_token(self, chunk, tag="bot_text"):
    ...
    def _generate_thread(self, prompt):
        ...
        current_response = ""
        reasoning_mode = self._show_reasoning.get()
        
        # Stream the response with reasoning if enabled
        for chunk in offline_distilled_stream(prompt, self._stop_evt, include_reasoning=reasoning_mode):
            if self._stop_evt.is_set():
                break
            current_response += chunk
            # Determine if this chunk belongs to reasoning or final answer
            # We could detect by content, but simpler: we use reasoning tag only for the reasoning part.
            # Since the generator now outputs reasoning first (if enabled), we can assume first part is reasoning.
            # But because we can't know the split in the stream easily, we'll tag based on presence of "💭" at start of line.
            # More robust: we could return separate streams, but for simplicity we'll tag lines starting with "💭" as reasoning.
            # Alternatively, we can add a marker in the generator. For now, we'll check the chunk content.
            # This is a heuristic and may mis‑tag if final answer contains 💭, but that's unlikely.
            if reasoning_mode and chunk.strip().startswith("💭"):
                self.after(0, self._post_token, chunk, "reasoning")
            else:
    ...
    def _generation_done(self, final_text=""):
        # Auto-extract Python code from the final answer and push to Sandbox
